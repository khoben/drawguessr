...
from services.wordprovider import FileWordProvider, FileWord

from config import config
from database.postgres import Database, PsycopgDatabase
from handlers import start, game, invite
...
def register_middlewares(dp: Dispatcher, db: Database) -> None:
    register_error_handler(dp)
    # register_throttle(dp, timeframe_sec=10, capacity=6)
...
def start_app() -> None:
    ...
    register_middlewares(dispatcher, database)
    ...
    game_controller = GameController(
        ...
        i18n,
        FileWordProvider(
            FileWord("en", "./services/words/en.txt", 1524),
        ),
    )
